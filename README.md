...
A collection that decomposes its elements into smaller chunks which is then stored in an array. Each element is then represented by indices into the array of parts.
...
The project is built with .NET SDK tools.
...
```
dotnet run --project tests/ComposedSetOfStrings/ComposedSetOfStrings.fsproj
```
